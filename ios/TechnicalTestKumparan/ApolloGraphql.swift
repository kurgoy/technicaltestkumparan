...
class ApolloGraphql : NSObject {
  ...
  @objc
  func query(_ pageSize: Int,cursor: String, successCallback: @escaping RCTResponseSenderBlock,failureCallback: @escaping RCTResponseSenderBlock) {
    apollo?.fetch(query: LaunchListQuery(pageSize: pageSize, cursor: cursor)) { result in
      switch result {
